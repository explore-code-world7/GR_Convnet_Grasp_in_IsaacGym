...
class Image:
    ...
    def copy(self):
        ...
        :return: Copy of self.
        """
        return self.__class__(self.img.copy())      # 通过 __class__ 属性，可以直接获取对象的类
...
# 从pcd file恢复图像，填补缺失值, 求梯度，cls的作用是什么?
class DepthImage(Image):
        ...
        with open(pcd_filename) as f:
            for l in f.readlines():
                ...
                if index is None:
                    ...
                    img[r, c] = np.sqrt(x ** 2 + y ** 2 + z ** 2)       # 从像素坐标，转化为距离

                else:
                    img[r, c] = float(ls[index])

        return cls(img / 1000.0)            # 10m归一化为[0,1]
    ...
    def inpaint(self, missing_value=0):
        ...
        self.img = cv2.copyMakeBorder(self.img, 1, 1, 1, 1, cv2.BORDER_DEFAULT)     # 添加一层边框
        ...
        self.img = cv2.inpaint(self.img, mask, 1, cv2.INPAINT_NS)       # cv2.INPAINT_NS 是修复算法的类型（Navier-Stokes 方法

        # Back to original size and value range.
        self.img = self.img[1:-1, 1:-1]                                 # 去掉外面的一层边框
