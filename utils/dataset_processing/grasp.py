...
class GraspRectangles:
        ...
        with open(fname) as f:
            while True:
                ...
                try:
                    ...
                    continue
        return cls(grs)                 # =函数所属的类(grs)=GraspRectangles(grs)
    ...
    # plot grasp rectangular's quality, angle and length value
    # 所有polygon_coords内的点都赋值上quality,angle and length values
...
class GraspRectangle:
    ...
    # output length of grasp: points[0]和points[1]
    @property
    def length(self):
        """
    ...
    # output width of grasp: points[1]和points[2]
    @property
    def width(self):
        """
    ...
    def compact_polygon_coords(self, shape=None):
        ...
        """
        return Grasp(self.center, self.angle, self.length / 3, self.width).as_gr.polygon_coords(shape)      # shape=output的尺寸

    # 计算grasprectangle和gr的IoU值
...
class Grasp:
    ...
    # 从grasp.angle,center,length,width输出抓取框
    @property
    def as_gr(self):
        """
    ...
    # compute  max iou between self.as_gr and grs
