...
class GraspDatasetBase(torch.utils.data.Dataset):
    ...
    # 从输入图像，输出标签框的四角生成的多边形区域的标记，角度，宽度
    def __getitem__(self, idx):
        ...
        if self.include_depth:
            depth_img = self.get_depth(idx, rot, zoom_factor)       # 经过变换后的depth

        # Load the RGB image
        if self.include_rgb:
            rgb_img = self.get_rgb(idx, rot, zoom_factor)           # 经过变换后的rgb

        # Load the grasps
        # 从文件中加载,dep/rgb,对应的抓取框
        bbs = self.get_gtbb(idx, rot, zoom_factor)                  # 抓取图像

        # 在抓取框中,输出ang_img, width_img, pos_img
        ...
        if self.include_depth and self.include_rgb:
            ...
            x = self.numpy_to_torch(rgb_img)        #[4,224,224]

        # 将pos_img,ang_img,width_img转为tensor
        pos = self.numpy_to_torch(pos_img)
        cos = self.numpy_to_torch(np.cos(2 * ang_img))  #[1,224,224]
