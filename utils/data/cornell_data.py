...
class CornellDataset(GraspDatasetBase):
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        # gtbbs代表一个抓取框,自带angle,center,length,width
        # 还有rotate,offset,zoom等变换函数
        gtbbs = grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx])
        center, left, top = self._get_crop_attrs(idx)
        # 这里gtbbs不是GraspRectangles类吗,没有rotate,offset.zoom函数亚~~?
        # 应该对gtbbs列表的每个元素用rotate,offset,zoom
        gtbbs.rotate(rot, center)           # 旋转变换
        gtbbs.offset((-top, -left))         # 平移变换
        gtbbs.zoom(zoom, (self.output_size // 2, self.output_size // 2))        # 缩放变换
        return gtbbs
